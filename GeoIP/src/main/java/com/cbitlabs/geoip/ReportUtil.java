...
public class ReportUtil {
	...
	// private static final String REPORT_SERVER_URL =
	// "http://cb101.public.cbitlabs.com";
	...
	public static JsonObject getWifiReport(Context c) {
		...
		return getReport(c, info.getSSID(), info.getBSSID(), getIPAddress(info), security, isEnterprise);
		...
		if (results != null) {
			for (ScanResult result : results) {
				if (!isDuplicateScanReport(c, result.BSSID)) {
					...
					JsonObject jsonReport = getReport(c, result.SSID, result.BSSID, NO_IP,
							getScanResultSecurity(result), scanResultIsEnterprise(result));
	...
	private static JsonObject getReport(Context c, String ssid, String bssid, String ip, String security,
	...
	public static String getReportAsString(JsonObject report) {
		String info = String.format("%s.%s.%s.%s.%s.%s", report.get("lat").getAsString(), report.get("lng")
				.getAsString(), report.get("ssid").getAsString().replace(".", "-"), report.get("bssid").getAsString(),
	...
	public static boolean isValidWifiReport(Context c, JsonObject report) {
		return report.entrySet().size() != 0 && WifiUtil.isWiFiConnected(c)
	...
	private static boolean isDuplicateReport(Context c, String prefKey, String bssid, boolean saveReport) {
